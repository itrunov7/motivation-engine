...
- Source states are computed per connection_mode from /corpora/{source_id}/manifest.json (D-013): an `api` source is connected iff the manifest exists with last_run.status = "success"; a `report` source is ingested iff additionally a data_files entry exists on disk; `manual` and `deferred` sources show their mode, never a connectivity status. Nothing is stored in sources.json.
